...
export default class SqliteDB {
  ...
  static addFileLog(fileLog: FileLog) {
    return new Promise((resolve) => {
      this.db.run(
        'INSERT INTO bms_logs (file_path, isUpdated, file_hash, created_at, size) VALUES (?, ?, ?, ?, ?)',
        [
          fileLog.file_path,
          fileLog.isUpdated,
          fileLog.file_hash,
  ...
  static getFileLogs(): Promise<any[]> {
    return new Promise((resolve, reject) => {
      this.db.all(
        'select * from bms_logs order by created_at desc',
        [],
        (err: any, rows: any) => {
          if (err) {
            reject(err);
  ...
  static findFile(fileLog: FileLog): Promise<any[]> {
    return new Promise((resolve, reject) => {
      this.db.all(
        'SELECT * FROM bms_logs WHERE file_path=? and file_hash=?',
        [fileLog.file_path, fileLog.file_hash],
        (err: any, rows: any) => {
          if (err) {
            reject(err);
          } else {
